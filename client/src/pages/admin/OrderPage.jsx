    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-10">
        ...
        <div className="flex gap-2 text-xs">
          <span className="flex items-center gap-1 bg-yellow-500/10 text-yellow-500 px-3 py-1 rounded-full border border-yellow-500/20 italic">
            ● Preparing
          </span>
          <span className="flex items-center gap-1 bg-blue-500/10 text-blue-500 px-3 py-1 rounded-full border border-blue-500/20 italic">
            ● Ready
          </span>
        </div>
      </div>
      ...
      <div className="grid gap-6">
        {orders.map((order) => (
          <div
            key={order._id}
            className="bg-white/[0.03] border border-white/10 rounded-[2rem] overflow-hidden group hover:border-indigo-500/30 transition-all"
          >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold">
                      {order.user?.name || "Anonymous Student"}
                    </p>
            ...
            <div className="bg-white/[0.02] px-6 py-3 border-t border-white/5 flex justify-between items-center">
              ...
              <span className="text-[10px] font-mono text-gray-600 flex items-center gap-1">
                <FiClock /> {new Date(order.createdAt).toLocaleTimeString()}
              </span>
            </div>
          </div>
        ))}
